...
public class FirstController {
    ...
    public FirstController(ParameterNamesModule parameterNamesModule) {
    ...
    * */
    ...
    /*
    파라미터가 여러개인 경우 Map 으로 한번에 처리할 수 있다.
    이때, Map 의 키는 form 의 name 속성값이 된다.
    * */
    ...
    @ModelAttribute 의 경우 커맨드객체를 생성하며 매개변수로
    전달 해 준 뒤 인스턴스를 model 에 담는다.

   경우에 따라 폼에서 입력한 값을 다음 화면으로 바로 전달해야 할 경우 발생하는데,
   이댸 유용하게 사용된다.

   @ModelAttribute("모델에 담을 key 값"_을 지정 할  수 있으며
   저장하지 않으면 타입의 앞급자들 소문자로 한 네이밍 규칙을 따른다.
   ->MemberDTO

   해당 어노티에션은 생략이 가능하지만 명시적으로 작성하는 것이 좋다
   이때 주의할점은 dto 와 입력받은 폼을 잘 맞추어야한다.
    ...
    * */
    ...
    /*
    @SessionAttributes 를 이용하여 session 에 값 담기
    클래스레벨에 @SessionAttributes 어노테이션을 이용하여 세션에 담을 key 값을 설정
    Model 영역에 해당 key 로 값이 추가되는 경우 session 에 자동으로 등록해준다.
    * */
    @PostMapping("login2")
