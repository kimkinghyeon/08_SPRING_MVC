...
클래스레벨에 URL 을 공통 부분을 이용해 설정하고 나면 매번 핸들러 메소드에 URL 의
...
public class ClassMappingTestController {
    ...
    public String modifyAndDelete(Model model) {
        model.addAttribute
                ("message","POST 방식의 주문 정보 수정과 주문 정보 삭제 공통 처리용 핸들러 메소드 호출함");
    return "page/mappingResult";
    }
    ...
    path variable 로 전달되는 {변수명} 값을 반드시 매개변수 명이랑 같아야 한다.
    만약 , 통일하지 않으면 @PathVariable("이름") 을 설정 해주어야한다.
    * */

    @GetMapping("/detail/{orderNo}")
    public String selectOrderDetail(Model model, @PathVariable int orderNo) {
        model.addAttribute("message",orderNo + "번 주문 상세 내용 조회용 핸들러 매소드 호출함");
        return "page/mappingResult";
    }
}
